...
class RobotUI(object):
    ...
    def read_file(self, path):
        # 讀取 JSON 檔案耗時大，選擇維護兩個變數 alarm_controller_list alarm_servo_list
    ...
    def feed_back(self):
        while True:
            ...
            self.client_feed.socket_dobot.setblocking(True)  # 設定為阻塞模式
            data = bytes()
    ...
    def display_error_info(self):
        # Try to use GetError interface first / 優先嘗試使用 GetError 介面
        try:
            ...
            if error_info and "errMsg" in error_info and error_info["errMsg"]:
                # Use new GetError interface / 使用新的 GetError 介面
        ...
        # Fallback to original method / 回退到原來的方法
        try:
    ...
    def form_error_new(self, error_data):
        """Handle error data from GetError interface / 處理 GetError 介面回傳的錯誤資料"""
        try:
